	...
	case dto.MetricType_COUNTER:
		return &dto.Metric{
			...
			Counter: &dto.Counter{
				...
				// Value: float64ptr(*b.Counter.Value),
			},
		}
...
func (a *aggate) parseAndMerge(r io.Reader) error {
	...
	// remove "transfer_" prefix

	inFamiliesNew := make(map[string]*dto.MetricFamily)
	for _, family := range inFamilies {
		name := strings.TrimPrefix(*family.Name, "transfer_")
		family.Name = &name
		inFamiliesNew[name] = family
	}
	inFamilies = inFamiliesNew
	fmt.Printf("%+v", inFamilies)
...
func main() {
	listen := flag.String("listen", ":8070", "Address and port to listen on.")
